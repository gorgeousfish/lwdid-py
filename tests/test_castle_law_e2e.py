"""
End-to-end tests for lwdid staggered mode using Castle Law data.

Tests Story E2-S1 Section 7 acceptance criteria.

Expected results (from Lee & Wooldridge 2023 Table 3):
- demean τ_ω ≈ 0.092
- detrend τ_ω ≈ 0.067

Castle Law Data:
- Source: castle.csv
- Outcome variable: lhomicide (log homicide rate)
- Unit identifier: sid (state integer ID, NOT state name)
- Time variable: year
- gvar variable: effyear (first treatment year, NaN = never treated)
- Cohorts: 2005, 2006, 2007, 2008, 2009
- Never treated: states with effyear = NaN
"""
...
class TestCastleDataValidation:
    """Validate Castle Law data structure and cohorts."""
    ...
    def test_cohort_structure(self, castle_data, castle_cohorts):
        """Check correct cohorts are identified."""
    ...
    def test_cohort_weights(self, castle_data, castle_cohorts):
        """Check cohort sizes for weight validation."""
...
class TestCastleDemean:
    """End-to-end test with demeaning transformation."""
    
    def test_demean_overall_effect(self, castle_data):
        """
        Test overall effect with demean transformation.
        
        Expected: τ_ω ≈ 0.092 (from Lee & Wooldridge 2023 Table 3)
        """
    ...
    def test_demean_cohort_effects(self, castle_data, castle_cohorts):
        """Test cohort-specific effects with demean."""
        from lwdid import lwdid
    ...
    def test_demean_cohort_time_effects(self, castle_data):
        """Test (g,r) specific effects with demean."""
        from lwdid import lwdid
...
class TestCastleDetrend:
    """End-to-end test with detrending transformation."""
    
    def test_detrend_overall_effect(self, castle_data):
        """
        Test overall effect with detrend transformation.
        
        Expected: τ_ω ≈ 0.067 (from Lee & Wooldridge 2023 Table 3)
        """
        from lwdid import lwdid
...
class TestCohortWeights:
    """Verify cohort weights match formula ω_g = N_g / N_treat."""
    
    def test_cohort_weights_formula(self, castle_data):
        """Verify cohort weights sum to 1 and match formula."""
        from lwdid import lwdid
...
class TestControlGroupVerification:
    """Verify control group behavior with Castle Law data."""
    
    def test_nyt_works_for_gr_effects(self, castle_data):
        """Not-yet-treated control works for (g,r) effects."""
        from lwdid import lwdid
    ...
    def test_nyt_auto_switches_for_overall(self, castle_data):
        """Not-yet-treated auto-switches to never-treated for overall."""
        from lwdid import lwdid
